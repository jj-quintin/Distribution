...
class AdminSupportController extends Controller
{
    ...
    public function adminSupportIndexAction()
    {
        ...
        return ['types' => $types, 'counters' => $counters];
    }
    ...
    public function adminNewTicketOpenAction(User $authenticatedUser, Ticket $ticket)
    {
        ...
        return new RedirectResponse(
            $this->router->generate('formalibre_admin_ticket_open', ['ticket' => $ticket->getId()])
        );
    }
    ...
    public function adminTicketOpenInterventionsAction(Ticket $ticket)
    {
        ...
        return ['ticket' => $ticket, 'totalTime' => $totalTime];
    }
    ...
    public function adminTicketInterventionStartAction(
        ...
        return new JsonResponse(['id' => $intervention->getId()], 200);
    }
        ...
        if (!is_null($timeType) && !is_null($startDate)) {
            ...
            if ($timeType === 0) {
                ...
                if (!is_null($endDate)) {
                    ...
                    $form->addError(
                        new FormError(
                            $this->translator->trans('end_date_is_required', [], 'support')
                        )
                    );
                }
            } elseif ($timeType === 1) {
                ...
                if (!is_null($duration)) {
                    ...
                    $form->addError(
                        new FormError(
                            $this->translator->trans('duration_is_required', [], 'support')
                        )
                    );
                }
            }
    ...
    public function adminTicketInterventionStatusEditFormAction(Intervention $intervention)
    {
        ...
        return ['form' => $form->createView(), 'intervention' => $intervention];
    }
